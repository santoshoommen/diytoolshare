...
/**
 * Enhanced EditListingPhotosForm with Image Classification
 * 
 * This component extends the original EditListingPhotosForm to include
 * automatic image classification and tool description suggestions.
 *
 * @component
 * @param {Object} props
 ...
 * @param {Function} props.onClassificationComplete - Callback when image classification is complete
 * @param {Function} props.onSuggestionAccepted - Callback when user accepts a suggestion
 ...
 * @returns {JSX.Element}
 */
export const EditListingPhotosFormWithClassification = props => {
  ...
  const handleClassificationComplete = (result) => {
    ...
    if (props.onClassificationComplete) {
      props.onClassificationComplete(result);
    }
  };

  const handleSuggestionAccepted = (suggestion) => {
    if (props.onSuggestionAccepted) {
      props.onSuggestionAccepted(suggestion);
    }
  };

  const handleSuggestionRejected = (result) => {
    if (props.onSuggestionRejected) {
      props.onSuggestionRejected(result);
    }
  };
        ...
        return (
          ...
          >
            ...
            {state.lastUploadedImage && (
              <ImageClassification
                imageFile={state.lastUploadedImage}
                onClassificationComplete={handleClassificationComplete}
                onSuggestionAccepted={handleSuggestionAccepted}
                onSuggestionRejected={handleSuggestionRejected}
                disabled={disabled}
                className={css.classificationContainer}
              />
            )}
...
export default EditListingPhotosFormWithClassification;
